"""Aggregates Kentucky-located listings from every taildraggers.com
...
ones whose location ends in ", KY".
...
Indiana/Iowa/Kansas repos' approach, with the state filter swapped to "KY".
"""
...
PAGE_TITLE = "Taildragger Ads in Kentucky"
STATE_ABBR = "KY"
...
_KENTUCKY_LOCATION_RE = re.compile(r",\s*KY\s*$", re.IGNORECASE)


def _is_kentucky(location: str) -> bool:
    return bool(_KENTUCKY_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        kentucky_rows = [row for row in rows if _is_kentucky(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(kentucky_rows)} in Kentucky")
        all_listings.extend(kentucky_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Kentucky listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Kentucky listings: {len(listings)}")
