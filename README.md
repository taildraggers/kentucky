...
Daily aggregator of Kentucky-located taildragger classified listings,
...
[Kansas](https://github.com/taildraggers/kansas) repos' approach, with
the state filter swapped to "KY".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, KY` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Kentucky"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/kentucky/`
...
```html
<iframe
  src="https://taildraggers.github.io/kentucky/"
  title="Taildragger Ads in Kentucky"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_KENTUCKY_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
